...
`GET /health` — public health check endpoint for uptime monitoring. Checks DB connectivity (`SELECT 1`) and Mollie API reachability (`payments.list(limit=1)`). Returns JSON `{"status": "ok"|"fout", "db": "ok"|"fout", "mollie": "ok"|"fout"|"niet_geconfigureerd"}` with HTTP 200 on success or 503 on failure. Bypasses maintenance mode (stays reachable when `onderhoudsmodus` is on, like `/webhook`). No rate limiting. Monitored externally via Uptime Kuma at `uptime.ipalm.nl/status/badeendjesracewapenveld`.
