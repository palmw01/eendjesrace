...
1. Public form (`/`, `templates/index.html`) → POST to `/bestellen`. The form is hidden server-side on first load; a **"Bestel hier"** CTA button reveals it with a slide-in animation. A subtle **"▲ Verbergen"** button inside the form collapses it again. If there are validation errors or pre-filled data (`vorig`), the form renders immediately without the CTA button. **Uitverkocht-modus**: zodra `beschikbaar <= 0` rendert de homepage een feestelijke `.uitverkocht-kaart` i.p.v. de bestelkaart (icoon 🎉, kop "Alle eendjes zijn verkocht!", verwijzing naar het diaconale project). De voortgangsbalk schakelt naar goud-groen (`.voortgang-fill.uitverkocht`), de `<title>` krijgt het prefix "🎉 Uitverkocht – " en de JSON-LD `availability` wordt `https://schema.org/SoldOut`. Het JS dat de auto-refresh draait detecteert de overgang (`d.uitverkocht && !isUitverkocht`) en doet `window.location.reload()` zodat reeds geopende sessies meteen de uitverkocht-versie zien. **Race-afgelopen modus**: een admin-schakelaar (`race_afgelopen` in `teller`) op `/admin/beheer` overrult zowel het bestelformulier als de uitverkocht-kaart en toont in plaats daarvan een `id="raceAfgelopenKaart"` blok met een 🏆-icoon, kop "De race is gevaren!" en een geordende lijst met de 12 winnende lotnummers (hardcoded in `templates/index.html`, geen namen). De voortgangsbalk wordt verborgen, `<title>` krijgt prefix "🏆 Race gevaren – ", JSON-LD `availability` blijft `SoldOut`, en de auto-refresh JS herlaadt geopende sessies wanneer `d.race_afgelopen !== isRaceAfgelopen`. Vallende eendjes triggeren op zowel `#uitverkochtKaart` als `#raceAfgelopenKaart`.
...
- **`teller`**: single row with `volgend_lot` (next ticket number), `max_eendjes` (total available, editable via admin), `max_per_bestelling` (per-order limit, editable via admin), `prijs_per_stuk`, `prijs_vijf_stuks`, `transactiekosten` (all editable via admin, seeded from env on first run), `onderhoudsmodus` (0/1, toggles maintenance mode, editable via admin), `race_afgelopen` (0/1, toggles winnaars-aankondiging op homepage, editable via admin — overrules `uitverkocht`)
- **`webhook_log`**: audit log of webhook calls
- **`audit_log`**: security audit trail — `tijdstip` (local datetime), `gebruiker` (admin username or NULL), `actie` (action key), `details` (free-text context), `ip` (remote addr). Populated by `schrijf_audit_log()` helper. Events: `login_succes`, `login_mislukt`, `account_geblokkeerd`, `logout`, `beheerder_aangemaakt`, `beheerder_verwijderd`, `wachtwoord_gewijzigd`, `instellingen_gewijzigd`, `opruimen`, `reset`, `mail_opnieuw`, `bestelling_gewijzigd`, `handmatige_bestelling`, `audit_log_gewist`. Viewable (last 200, newest first) on `/admin/beheer`. **Not cleared by database reset** — the reset action itself is logged before wiping. Cleared separately via `POST /admin/audit-wissen`, which writes an `audit_log_gewist` entry AFTER the delete so there's always a trace.
- **`beheerders`**: admin accounts — `gebruikersnaam` (unique), `wachtwoord_hash` (Werkzeug PBKDF2), `aangemaakt_op`, `laatste_inlog` (nullable, set on each successful login), `sessie_versie` (INTEGER, incremented on password change to invalidate all other sessions), `mislukte_pogingen` (INTEGER, consecutive failed login counter), `geblokkeerd_tot` (TEXT ISO timestamp, account locked for 15 min after 10 consecutive failures), `totp_geheim` (TEXT nullable, base32 TOTP secret), `totp_actief` (INTEGER, 0/1 — whether 2FA is required at login). Seeded on first start from `ADMIN_USER`/`ADMIN_PASS` env vars **only if the table is empty**. Multiple accounts supported; manageable via the admin panel without redeployment. The `laatste_inlog` and `totp_actief` columns are shown in the beheerders table on `/admin/beheer`.
...
- `POST /admin/reset` — full database reset (wipes all orders + webhook_log, resets `volgend_lot` to 1, resets SQLite autoincrement so IDs start at 1 again). **Does not delete admin accounts** (`beheerders` table is untouched) **and does not clear `audit_log`** (the reset event itself is logged). Requires typing `RESET` as confirmation. Has a JS confirm dialog as second safeguard. Redirects to `/admin/beheer`.
- `POST /admin/audit-wissen` — clears the entire `audit_log` table (resets autoincrement), then writes one `audit_log_gewist` entry so the wipe action is always traceable. Redirects to `/admin/beheer`.
- `GET /admin/2fa/instellen` — 2FA setup page: shows QR code (base64 PNG of otpauth:// URI) and manual base32 key. If `totp_actief=1`, shows "already enabled" message instead.
- `POST /admin/2fa/nieuw-geheim` — generate a fresh TOTP secret (stored unactivated in DB), redirect to setup page.
- `POST /admin/2fa/bevestigen` — verify first TOTP code and set `totp_actief=1`. Uses `valid_window=1` (±30s clock skew tolerance). Logs `2fa_ingeschakeld` to audit log.
- `POST /admin/2fa/uitschakelen` — verify current TOTP code and set `totp_actief=0`, clear `totp_geheim`. Logs `2fa_uitgeschakeld` to audit log.
- `GET/POST /admin/login/totp` — second login step when 2FA is active. Reads `session["totp_pending_gebruiker"]`, verifies code, calls `_voltooi_login()` on success. Rate-limited at 5/min.
...
- `POST /admin/beheerder-wachtwoord-reset/<id>` — reset the password of another admin account (cannot be used on own account); requires new password ≥ 12 chars and matching confirmation. Increments `sessie_versie` (invalidating existing sessions of that user) and clears `mislukte_pogingen`/`geblokkeerd_tot` so a blocked account can log in immediately. Logs `wachtwoord_reset` (orange/waarschuwing) to the audit log. Accessible via the "Reset wachtwoord" button in the beheerders table on `/admin/beheer`. Redirects to `/admin/beheer`.
...
`GET /api/beschikbaar` — public JSON endpoint returning `verkocht`, `beschikbaar`, `max_eendjes`, `max_per_bestelling`. Used by the homepage auto-refresh (every 30s). Bypasses maintenance mode so uptime monitors keep receiving valid responses.

`GET /health` — public health check endpoint for uptime monitoring. Checks DB connectivity (`SELECT 1`) and Mollie API reachability (`payments.list(limit=1)`). Returns JSON `{"status": "ok"|"fout", "db": "ok"|"fout", "mollie": "ok"|"fout"|"niet_geconfigureerd"}` with HTTP 200 on success or 503 on failure. Bypasses maintenance mode (stays reachable when `onderhoudsmodus` is on, like `/webhook` and `/api/beschikbaar`). No rate limiting. Monitored externally via Uptime Kuma at `uptime.ipalm.nl/status/badeendjesracewapenveld`.
...
`tests/test_app.py` stubs out Mollie, Resend, Flask-WTF CSRF, and Flask-Limiter so tests run with only Flask installed. Tests cover pricing, input validation, database operations, atomic transactions, email sending, webhook processing, admin routes, `max_per_bestelling`/`max_eendjes` settings, price settings (`prijs_per_stuk`/`prijs_vijf_stuks`/`transactiekosten`) via admin, CSV injection escaping, CSV filename timestamp, CSV header columns, email header content (afbeelding/kleur/datum/locatie/tijd/praktische-info-icoon), email validation in wijzig_bestelling, opruimen, paginering, server-side statusfilter, CSP nonces, Permissions-Policy, session permanence, `saniteer_log`, legal pages (`/privacy`, `/voorwaarden`), footer content on `/betaald/<mollie_id>` and error pages (`fout.html`), collapsible `<details>` sections in admin, multi-account admin, password management and password reset for other accounts (`TestBeheerderAccounts`, including `laatste_inlog`, reset validation, session invalidation, blokkering opheffing tests), security headers (`TestSecurityHeadersAanvullend`), email formatting variants (`TestBevestigingsmailOpmaakvarianten`), input boundary values (`TestValideerInvoerGrenswaardes`), atomic ticket assignment edge cases (`TestWijsLotnummersToeAanvullend`), admin page username display, password change persistence, betaald-page fallback errors, bestellen edge cases, admin settings validation, handmatige bestelling telefoon, webhook audit log, SEO meta-tags/robots.txt/sitemap.xml (`TestSeoEnRobots`), beheer-paginasplitsing en filterbalk-layout (`TestAdminBeheerPagina`), onderhoudsmodus aan/uit/503/admin-bypass/webhook-bypass/api-beschikbaar-bypass (`TestOnderhoudsmodus`), sponsorstrip statisch/scroll/drie-rijen/volgorde/bestandstype-filter (`TestSponsorStrip`), vallende eendjes animatie/accordion/afzendernaam/projectblokje (`TestRecenteWijzigingen`), setup-pagina token/validatie/aanmaken/invalidatie/tokenbestand (`TestSetupPagina`), betaald-tussenroute redirect/onbekend-id/null-mollie_id (`TestBetaaldRedirect`), instellingen transactie-atomiciteit en DB-foutafhandeling (`TestAdminInstellingenTransactie`), audit-log aanmaken/wissen/zichtbaar/ip-adres/CF-Connecting-IP (`TestAuditLog`), 2FA instellen/activeren/uitschakelen/login-flow (`TestTweeFactorAuth`), gezondheidscheck endpoint/Mollie-fout/onderhoudsmodus-bypass/statuslink in fout- en onderhoudspagina (`TestGezondheidsCheck`), betaald-paginatitel per status/webhook-alarm-stat/verkopen-per-dag-barchart (`TestAdminStatistieken`), CTA-knop/formulier-verbergen/formulier-zichtbaar-na-fout (`TestHomepageEnApi`), uitverkocht-modus kaart/formulier-verborgen/40-vallende-eendjes/titel/SoldOut-JSON-LD/voortgangsbalk-label/`uitverkocht`-API-veld (`TestUitverkochtModus`), race-afgelopen modus toggle/winnaarsblok/12-lotnummers/geen-namen/bestelformulier-verborgen/uitverkocht-overrule/voortgangsbalk-verborgen/titel/SoldOut-JSON-LD/vallende-eendjes/API-veld/audit-log (`TestRaceAfgelopenModus`). The test database uses `/tmp/eendjes_test.db` (reset before each test class). `maak_db()` includes all teller columns and seeds the `beheerders` table (incl. `laatste_inlog`). `conftest.py` cleans up the test database + WAL/SHM files before pytest starts (required for Python 3.14 + SQLite WAL mode). Total: 541 tests. Tests use `self.client.post("/admin/logout")` (POST-only since CSRF-logout fix).
...
- **≥ 5 sponsors**: drie scrollende rijen (`sponsor-baan` × 3, `sponsor-rij` per baan). De lijst wordt met Jinja's `slice(3)` filter in drie delen gesplitst. Rij 1 en 3 scrollen links, rij 2 (middelste) scrolt rechts (tegengesteld). Elk gedupliceerd voor naadloze loop. Pauzeert bij hover.
...
Op de betaald-pagina (`/betaald/<mollie_id>`) vallen bij `status_klasse == 'succes'` 28 badeendjes van boven het scherm naar beneden. Op de homepage (`/`) vallen er 40 badeendjes wanneer alle loten verkocht zijn (`uitverkocht == True`, gedetecteerd via `document.getElementById('uitverkochtKaart')`). Geïmplementeerd via:
- CSS `@keyframes valEend` (in `betaald.html` altijd aanwezig in `<style>`; in `index.html` altijd aanwezig in `<style>` zodat het script bij overgang-via-reload direct kan starten)
- JavaScript (met CSP-nonce) dat `<img class="vallend-eendje">` elementen aanmaakt met willekeurige x-positie (%), grootte (28–48px), duur (2.4–4.6s) en vertraging (0–3s op betaald, 0–3.5s op homepage). Elementen verwijderen zichzelf na `animationend`.
...
- **Client IP detection**: `get_client_ip()` returns the real visitor IP. When the app runs behind Cloudflare, the `CF-Connecting-IP` header contains the actual client IP; otherwise falls back to `request.remote_addr` (which ProxyFix has already corrected for the Railway load balancer). Used in all audit log and security log calls.
...
- **`/betaald/<mollie_id>`**: validates that `mollie_id` starts with `tr_` and is ≤ 64 chars before hitting the DB or Mollie API; returns 404 otherwise. The `<title>` is dynamisch per `status_klasse`: `succes` → "✅ Betaald", `wacht` → "⏳ Betaling in behandeling", `fout` → "❌ Betaling mislukt", overige → "⚠️ Betaalstatus".
