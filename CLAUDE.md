...
- `POST /admin/instellingen` — update `max_eendjes`, `max_per_bestelling`, `prijs_per_stuk`, `prijs_vijf_stuks`, `transactiekosten`, `notificatie_email`, and/or `onderhoudsmodus` in the `teller` table. Validates that `max_eendjes` cannot be set below the number of already sold tickets, prices must be > 0, transactiekosten >= 0, notificatie_email must be a valid address or empty. `onderhoudsmodus` is a checkbox (present = on, absent = off). When set, a silent copy of every confirmation email is sent to the notificatie_email address with subject `[Kopie] Bestelling … – Badeendjesrace!`. **All fields are validated before any DB write** — if any field is invalid, nothing is saved (atomic all-or-nothing). Redirects to `/admin/beheer`.
